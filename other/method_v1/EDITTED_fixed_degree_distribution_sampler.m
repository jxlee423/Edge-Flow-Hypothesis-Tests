function [edge_to_endpoints,A] = ADJUSTED_fixed_degree_distribution_sampler(N,degree_dist);
...
while stop == 0
    ...
    %% [OLD] make edge to endpoints
    % edge_to_endpoints(:,1) = stub_assignment(stub_list_left);
    % edge_to_endpoints(:,2) = stub_assignment(E + stub_list_right);

    %% [NEW-ADDED] Initialize edge_to_endpoints
    edge_to_endpoints = zeros(0, 2);  % Ensure edge_to_endpoints is an empty 2-column matrix

    %% [NEW-ADDED] Create edge_to_endpoints and avoid duplicates
    for idx = 1:E
        node1 = stub_assignment(stub_list_left(idx));
        node2 = stub_assignment(E + stub_list_right(idx));

        % Ensure no self-loops or duplicate edges
        if node1 ~= node2 && ~any(ismember(sort([node1, node2]), edge_to_endpoints, 'rows'))
            edge_to_endpoints = [edge_to_endpoints; sort([node1, node2])]; 
        end
    end

    %% [NEW-ADDED]Recompute E based on unique edges
    E = size(edge_to_endpoints, 1);

    %% find self loops
    self_loops = find(edge_to_endpoints(:, 1) - edge_to_endpoints(:, 2) == 0);

    %% rewire self loops for initial seed
    while ~isempty(self_loops)
