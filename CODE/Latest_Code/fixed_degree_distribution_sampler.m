...
while stop == 0
    ...
    %% [DELETED] make edge to endpoints
    ...
    % Create edge_to_endpoints and avoid duplicates
    for idx = 1:E
        ...
        if node1 ~= node2 && ~any(ismember(sort([node1, node2]), edge_to_endpoints, 'rows'))
            edge_to_endpoints = [edge_to_endpoints; sort([node1, node2])];  % Sort to ensure consistent ordering
        end
    end
